...
class Model(object):
    ...
    def calculate(self, smiles_list): # <-- EDIT: rename if model does not do predictions (e.g. it does calculations)
...
class Service(BentoService):
    ...
    def calculate(self, input: List[JsonSerializable]): # <-- EDIT: rename if necessary 
        ...
        output = self.artifacts.model.calculate(smiles_list) # <-- EDIT: rename if necessary
        return [output]
